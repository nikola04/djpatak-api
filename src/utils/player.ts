    ...
    if (
      ...
      if (playTrackResp.state == PlayerState.QueueEnd) {
        ...
        if (repeatResp && repeatResp.state == PlayerState.Playing) {
          return events?.onNextSong
            ? events.onNextSong(repeatResp.track)
            : null;
        }
      ...
      if (playTrackResp.state == PlayerState.Playing) {
        return events?.onNextSong
          ? events.onNextSong(playTrackResp.track)
          : null;
      }
    }
  });
...
async function playQueueFromStart(
  ...
  | { state: Exclude<PlayerState, PlayerState.Playing>; track: null }
  | false
> {
  ...
  try {
    ...
    if (resource.volume && connection.playerPreferences) {
      ...
      resource.volume.setVolume(
        connection.playerPreferences.volume.getVolume(),
      ); // update vol
...
export async function playQueueTrack(
  connection: VoiceConnection,
