    ...
    wss.handleUpgrade(req, socket, head, (ws: WebSocket) => {
      ...
      ws.on("message", (message) => {
        try {
          ...
          if (event !== "subscribe" || !data?.playerId)
            return unsubscribeSocket(socketId);
...
export function emitEvent(
  event: "new-queue-songs",
  playerId: string,
  tracks: QueueTrack[],
): void;
export function emitEvent(
  event: "remove-queue-song",
  playerId: string,
  queueId: string,
): void;
export function emitEvent(
...
export function emitEvent(
  event: EventType,
  playerId: string,
  data?: QueueTrack | QueueTrack[] | Repeat | number | string,
): void {
