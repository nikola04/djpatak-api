  ...
  async (req: Request, res: Response) => {
    ...
    if (!account)
      return res
        ...
        .json({
          status: "error",
          error: "User has no Discord Account Connected",
        });
    ...
    if (!Array.isArray(userResponse)) {
      ...
      return res
        .status(500)
        .json({ status: "error", error: "Error while getting User Guilds" });
    }
    const userGuilds = userResponse as PartialDiscordGuild[];
    if (botGuilds.size == 0) return res.json([]);
    const userGuildsWithBot = userGuilds.filter(
      (userGuild: PartialDiscordGuild) => botGuilds.has(userGuild.id),
    );
