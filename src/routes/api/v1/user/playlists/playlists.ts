...
router.post("/", async (req: Request, res: Response) => {
  ...
  if (!isValidPlaylistName(trimmedName))
    return res.status(400).json({
      error:
        "Playlist Name must be between at 2 and 24 alphabet characters and emojis only",
    });
  if (!isValidPlaylistDescription(description))
    return res.status(400).json({
      error:
        "If provided, Playlist Description can have maximum of 100 characters",
    });
  try {
    ...
    if (exist != null)
      return res
        .status(409)
        .json({ error: "You have already created playlist with that name" });
...
router.delete("/:playlistId", async (req: Request, res: Response) => {
  ...
  try {
    ...
    await Promise.all([
      ...
      PlaylistTrackModel.deleteMany({ playlistId }),
    ]);
