...
router.get('/history', ratelimit({ ratelimit: 1000, maxAttempts: 1 }), async (req: Request, res: Response) => {
	const userId = req.userId;
	try {
		const results: IUserSearch[] = await UserSearchModel.find({ userId }).limit(500).lean();
		return res.json({ status: 'ok', results });
	} catch (err) {
		console.error('GET api/v1/tracks/search/history', err);
		return res.status(500).json({ error: 'Internal server error' });
	}
});

router.delete('/history/:id', ratelimit({ ratelimit: 1000, maxAttempts: 1 }), async (req: Request, res: Response) => {
	const userId = req.userId;
	const historyId = req.params.id;
	if (!isValidObjectId(historyId)) return res.status(400).json({ error: 'History ID is Not Valid' });
	try {
		await UserSearchModel.deleteOne({ userId, _id: historyId });
		return res.json({ status: 'ok' });
	} catch (err) {
		console.error('DELETE api/v1/tracks/search/history/:id', err);
		return res.status(500).json({ error: 'Internal server error' });
	}
});

router.get('/:query', ratelimit({ ratelimit: 1000, maxAttempts: 2 }), async (req: Request, res: Response) => {
	const userId = req.userId;
