...
router.delete('/:queueId', async (req: Request, res: Response) => {
	...
	try {
		...
		const channel = member.voice.channel!;
		const connection = getVoiceConnection(playerId);
		if (!connection) return res.status(403).json({ status: 'error', error: 'Bot is not in voice channel' });
		if (guild.members.me?.voice.channelId != channel.id)
			return res.status(403).json({ status: 'error', error: 'You must be in same channel as Bot' });
		if (!(await removeTrackByQueueId(playerId, queueTrackId))) return res.json({ status: 'error' });
...
router.get('/current', async (req: Request, res: Response) => {
	...
	try {
		...
		if (guild.members.me?.voice.channelId != channel.id)
			return res.status(403).json({ status: 'error', error: 'You must be in same channel as Bot' });
		const connection = getVoiceConnection(playerId);
		if (!connection) return res.status(400).json({ status: 'error', error: 'Player is not connected' });
