...
router.post('/next', async (req: Request, res: Response) => { // Getting current track
    ...
    try{
        ...
        if(state == PlayerState.NoTrack) 
            return res.json({ status: 'error', error: 'Track Not Found, Current is removed from Queue', playerStatus: connection.player?.state.status === AudioPlayerStatus.Playing ? 'playing' : 'paused', queueTrack: null })
        if(state == PlayerState.QueueEnd)
