...
import { PlayerState, playQueueTrack, initializePlayer, initializeDefaultPlayerEvents } from '@/utils/player';
...
router.post('/:playlistId', async (req: Request, res: Response) => {
	...
	try {
		...
		if (!playlist) return res.status(404).json({ status: 'error', error: 'Playlist is not found' });
		if (playlist.ownerUserId != req.userId) return res.status(403).json({ status: 'error', error: 'You are not a playlist owner' });
		...
		if (state == PlayerState.NoStream)
			return res.json({
				...
				playerStatus: connection.player?.state.status === AudioPlayerStatus.Playing ? 'playing' : 'paused',
				queueTrack: null,
			});
