...
router.get('/', async (req: Request, res: Response) => {
	...
	if (!code || typeof code !== 'string') return res.status(400).json({ status: 'error', error: 'Code Not Found' });
	...
	const oAuthData: OAuthData | null = await fetch('https://discord.com/api/v10/oauth2/token', {
		...
		.then((res) => res.json())
		.catch(() => null);
	if (!oAuthData || !oAuthData.access_token) return res.status(400).json({ status: 'error', error: 'Code Not Valid' });
	const userResult: DiscordUserResult | null = await fetch('https://discord.com/api/users/@me', {
		...
		.then((res) => res.json())
		.catch(() => null);
	if (!userResult || !userResult.id) return res.status(500).json({ status: 'error', error: 'Error while getting Discord User' });
	// Create Account if doesnt exists
	const account =
		(await Account.findOne({ providerAccountId: userResult.id, provider: 'discord' })) ?? (await newDiscordAccountUser(userResult, oAuthData));
	await Promise.all([
